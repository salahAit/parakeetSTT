...
# Configure local temporary directory on SSD to avoid /tmp space limits
PROJECT_ROOT = Path(__file__).parent.absolute()
LOCAL_TEMP_DIR = PROJECT_ROOT / "temp_cache"
LOCAL_TEMP_DIR.mkdir(exist_ok=True)
os.environ["TMPDIR"] = str(LOCAL_TEMP_DIR)
os.environ["NEMO_CACHE_DIR"] = str(LOCAL_TEMP_DIR)
...
def extract_audio_to_memory(video_path):
    ...
    try:
        ...
        if process.returncode != 0:
            ...
            return None
        print(
            f"[+] Extracted {len(audio_data)/(1024*1024):.2f} MB to RAM in {time.time() - start_time:.2f}s"
        )
        return audio_data
...
def process_file(file_path, model):
    ...
    try:
        ...
        with torch.no_grad(), torch.autocast("cuda"):
            ...
            transcriptions = model.transcribe(temp_files, batch_size=48)
        ...
        with open(json_path, "w", encoding="utf-8") as f:
            json.dump(
                [
                    ...
                    for start, end, text in small_segments
                    if text.strip()
                ],
                f,
                indent=2,
